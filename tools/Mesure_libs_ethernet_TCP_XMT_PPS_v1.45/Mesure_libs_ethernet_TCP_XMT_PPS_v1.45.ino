...
void http_client_gestion()
{
  ...
  char http_buffer[HTTP_BUFFER_SIZE], http_message_in[5];
  ...
  if ( sortir == 5 ) {           // Normalement tout est ok
    memcpy (http_message_in, http_buffer + 6, 2);
    http_message_in[2] = 0;
    Serial.println(http_buffer);
    if ( strcmp(http_message_in, "50" ) == 0 ) send_buffer(50);
    else if ( strcmp(http_message_in, "15" ) == 0 ) send_buffer(150);
    else if ( strcmp(http_message_in, "40" ) == 0 ) send_buffer(400);
  }
  http_client.flush();
 // delay(1);
  http_client.stop();
