...
// *****************************************************************
// init() search which chip is connected and initialize it as desired
// with the right number of sockets and corresponding buffer sizes
// return the model of the chip found, 0 if none found
// *****************************************************************
uint8_t W5100Class::init(void)
{
	maxindex = MAX_SOCK_NUM;
	OffsetAddressMapping = false;
...
bool W5100Class::softReset(void)
{
	uint8_t count=0;
...
uint16_t W5100Class::write(uint16_t addr, const uint8_t *buf, uint16_t len)
{
	...
	if (chip == 52) {
		...
		if (len <= SPI_BUFFER_SIZE - 4) {
			...
			for ( i = 0; i < len; i += SPI_BUFFER_SIZE) {
				memcpy(cmd, buf + i, SPI_BUFFER_SIZE);
				SPI.transfer(cmd, SPI_BUFFER_SIZE);
			}
			for ( ; i < len; i++) {
				SPI.transfer(buf[i]);
			}
	...
	// chip == 55
		...
		if (addr < 0x100) {
			...
			#endif
		} //else {		// Why write in RX buffers ?
		if (len <= SPI_BUFFER_SIZE - 3) {
			for (uint8_t i = 0; i < len; i++) {
				cmd[i + 3] = buf[i];
			}
...
void W5100Class::execCmdSn(SOCKET s, SockCMD _cmd)
{
	...
	uint32_t stopWait = millis() + 5;
	while ( readSnCR(s) ) {
		if ( millis() > stopWait ) return;
	}
}
