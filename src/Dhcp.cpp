...
//	Serial.println("request_DHCP_lease");
    // Generate an initial transaction ID
    _dhcpTransactionId = 4096 + (millis() & 0x3FFF);
    ...
    //Serial.println("We're done with the socket now");
...
/***************************************************************************/
bool DhcpClass::send_DHCP_MESSAGE(uint8_t messageType, uint32_t startTime)
{
    uint8_t buffer[32];
    memset(buffer, 0, 32);
    ...
    if (_dhcpUdpSocket.beginPacket(dest_addr, DHCP_SERVER_PORT) != 1 ) {
        //Serial.println("DHCP transmit error");
        return false;
    }

    uint16_t secondsElapsed = (millis() - startTime ) / 1000;	// Pierre 12/12/19
    ...
    //Serial.println("_dhcpUdpSocket.endPacket");
    if ( _dhcpUdpSocket.endPacket() == 1 ) return true;
    return false;
}
...
uint8_t DhcpClass::parseDHCPResponse(uint32_t& transactionId)
{
    ...
    //Serial.println("_dhcpUdpSocket.parsePacket");
