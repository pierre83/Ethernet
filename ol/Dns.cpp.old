...
{
	dDNSServer = aDNSServer;
	
	// Check we've got a valid DNS server to use
	if (dDNSServer == IPAddress((uint32_t)0) ) {
...
int DNSClient::getHostByName(const char* aHostname, IPAddress& aResult, uint16_t timeout)
{
	...
	uint16_t port = randomNumber();
	int ret = iUdp.begin(port);
	if ( ret == SUCCESS ) {
		// Successful get socket
		while ( retries < MAX_RETRIES ) {
		    // Open DNS server connexion
			ret = iUdp.beginPacket(dDNSServer, DNS_PORT);
			if ( ret == SUCCESS ) {
				// Build the request
				...
				if ( ret == SUCCESS ) {
					...
					while ( millis() < stopWait ) {
						packetSize = iUdp.parsePacket();
						if ( packetSize > 0 ) {
							// We've got something
							if ( ( iUdp.remoteIP() != dDNSServer ) || ( iUdp.remotePort() != DNS_PORT ) ) {
...
uint32_t stop = millis();
    Serial.print("@IP\t");			Serial.println(aResult);
    Serial.print("Retries\t");		Serial.println(retries);
    Serial.print("Temps\t");		Serial.println(stop - start);
...
// *******************************************************
// Create a pseudo random number  1024 <= n <= 1779
// *******************************************************
uint16_t DNSClient::randomNumber()
{
	return 1024 + (millis() & 0x03FF);
}
...
uint16_t DNSClient::BuildRequest(const char* aName)
{
	...
	uint16_t iRequestId = randomNumber(); // generate a random ID
...
uint16_t DNSClient::ProcessResponse(uint16_t iRequestId, IPAddress& aAddress)
{
	...
	if ( (dDNSServer != iUdp.remoteIP()) || (iUdp.remotePort() != DNS_PORT) ) {
		// It's not from who we expected
		return INVALID_SERVER;
	}
